...
function createData(full_name,phone, store_id, passport_series, passport_number, issue_date, who_issued) {
return {
full_name,
phone,
store_id,
...
const newSelecteds = rows.map((n) => n.full_name);
...
.map((row, index) => {
const isItemSelected = isSelected(row.full_name);
...
<TableCell align="right">{row.passport_series}</TableCell>
<TableCell align="right">{row.territory}</TableCell>
<TableCell align="right">{row.issue_date}</TableCell>
<TableCell align="right">{row.who_issued}</TableCell>
</TableRow>
);
})}
{emptyRows > 0 && (
